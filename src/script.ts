...
const setupElements = (
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    checkbox.addEventListener(`change`, () => {
      if (lastModule !== container && element.classList.contains(`card`)) {
        if (lastModule) {
          if (container.id !== lastModule.id) {
            lastModule.getElementsByTagName(`input`)[0].checked = false
            lastModule.classList.remove(`active`)

            const lastModuleCard = lastModule.querySelector(`label.card.active`)
            if (lastModuleCard) {
              lastModuleCard.getElementsByTagName(`input`)[0].checked = false
              lastModuleCard.classList.remove(`active`)
              lastModuleCard.parentElement?.classList.remove(`hide`)
            }

            toggleParcels(
              <HTMLLabelElement>lastModule.getElementsByClassName(`card`)[0]
            )(false)
            if (element.classList.contains(`card`)) {
              setMessage(lastModule)(element.getAttribute(`message`) ?? ``)
            } else {
      ...
      if (last !== element) {
        if (last) {
          if (
            !element.classList.contains(`card`) ||
            container.querySelectorAll(`label.card.active`).length > 0
          ) {
      ...
      if (checkbox.checked) {
        if (element.classList.contains(`card`)) {
          ...
          const parcelsContainer = container.getElementsByClassName(`method`)[0]
